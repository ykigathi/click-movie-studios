...
import { ChevronLeft, ChevronRight, Filter, Grid, List, Star, TrendingUp, Calendar, Play, X } from 'lucide-react'
...
export const Movies: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'popular' | 'all' | 'newest' | 'nowPlaying' | 'topRated' | 'upcoming'>('all')
  ...
  useEffect(() => {
    if (activeTab === 'all' && filtersApplied) {
      fetchAllMovies(1, filters)
    }
  }, [filters, filtersApplied, activeTab])
  ...
  const handleTabChange = (tab: string) => {
    setActiveTab(tab as 'popular' | 'all' | 'newest' | 'nowPlaying' | 'topRated' | 'upcoming')
  }
  ...
  const handlePageChange = (page: number) => {
    if (activeTab === 'popular') {
      ...
      fetchAllMovies(page, filtersApplied ? filters : undefined)
    } else if (activeTab === 'nowPlaying') {
      ...
      >
        ...
        <Tabs value={activeTab} onValueChange={handleTabChange}>
          <TabsList className="grid w-full grid-cols-6">
            ...
            <TabsTrigger value="nowPlaying" className="flex items-center gap-2">
          ...
          <TabsContent value="nowPlaying" className="mt-6">
            {renderMoviesGrid(nowPlayingMovies, nowPlayingLoading, nowPlayingError)}
            {renderPagination(nowPlayingPage, nowPlayingTotalPages)}
          </TabsContent>

          <TabsContent value="topRated" className="mt-6">
            {renderMoviesGrid(topRatedMovies, topRatedLoading, topRatedError)}
            {renderPagination(topRatedPage, topRatedTotalPages)}
          </TabsContent>

          <TabsContent value="upcoming" className="mt-6">
            {renderMoviesGrid(upcomingMovies, upcomingLoading, upcomingError)}
            {renderPagination(upcomingPage, upcomingTotalPages)}
          </TabsContent>
